...
В прошивке ищем функцию '''cpp SolidWhite() ''' (для изменения/добавления световой температуры), или '''cpp SolidColor() ''' для добавления/изменения цвета или градиента.
Если хотим добавить световую температуру ('''cpp SolidWhite() '''):
...
2) добавляем после последнего '''cpp break; ''' конструкцию:
'''cpp
case x: strip.fill(mRGB(200, 240, 255));
        break;
'''
Вместо '''cpp x ''' подставить следующий номер подфункции.

2.1) Если же хотим поменять, то в любой строке в скобки подставляете полученное значение цвета (там, где '''cpp mRGB(x, x, x) ''')
3) В строке '''cpp if (subMode_mode5 > 3) subMode_mode5 = 0; ''' изменить цифру 3 на количество получившихся режимов.

Чтобы добавить цвет в функцию '''cpp SolidColor() ''' можно воспользоваться любой цветовой палитрой, например, [такой](https://g.co/kgs/9ZMMEp).
...
3) Не забываем поменять в строке '''cpp if (subMode_mode6 > 8) subMode_mode6 = 0; ''' количество подрежимов на итоговый.

Чтобы добавить градиент:
1) Градиент возможен из двух цветов. Подбираем необходимые параметры RGB в палитре.
2) находим строку '''cpp strip.fillGradient(0, NUMLEDS, mRGB(255, 0, 0), mRGB(0, 0, 255));  '''. Третий и четвертый параметры отвечают за начальный и конечный цвет градиентв.
