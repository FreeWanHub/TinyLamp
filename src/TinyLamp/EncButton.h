...
public:
    ...
    void setStepTimeout(int tout) {
        _stepT = tout; // сдвиг заменен на прямую установку, увеличивает занимаемую память, но позволяет установить любой период -- FreeWan
    }
        ...
        if (_btnState) {                                                // кнопка нажата
            ...
            if (!readF(3)) {                                            // и не была нажата ранее
                ...
                if (!readF(4)) {                                        // и удержание ещё не зафиксировано
                    ...
                    if (debounce > (uint16_t)(_stepT)) {           // таймер степа
                        EBState = 7;                                    // сигналим
